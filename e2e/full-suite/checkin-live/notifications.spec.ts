...
test('donor can mark a notification as read', async ({ page, seedRefs }) => {
  const session = await loginAs('donor')
  const list = await page.request.get(`${API_URL}/notifications?event_id=${seedRefs.liveEventId}`, {
    headers: { Authorization: `Bearer ${session.accessToken}` },
  })
  ...
  const response = await page.request.post(`${API_URL}/notifications/${notificationId}/read?event_id=${seedRefs.liveEventId}`, {
    headers: { Authorization: `Bearer ${session.accessToken}` },
  })
