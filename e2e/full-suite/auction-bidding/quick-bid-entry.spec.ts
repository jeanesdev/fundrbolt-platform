...
test('quick bid with a valid paddle and table is accessible', async ({ page, seedRefs }) => {
  ...
  const response = await page.request.post(`${API_URL}/admin/events/${seedRefs.liveEventSlug}/quick-bid-entry`, {
    headers: { Authorization: `Bearer ${session.accessToken}` },
    data: { bidder_number: 101, table_number: 1, amount: 100 },
  })
...
test('quick bid with an unassigned paddle returns an error', async ({ page, seedRefs }) => {
  ...
  const response = await page.request.post(`${API_URL}/admin/events/${seedRefs.liveEventSlug}/quick-bid-entry`, {
    headers: { Authorization: `Bearer ${session.accessToken}` },
    data: { bidder_number: 999999, table_number: 1, amount: 100 },
  })
