...
**Feature Branch**: `016-i-have-several`
**Created**: 2026-01-19
**Status**: Draft
...
1. **Given** the theme system is configured with primary colors (Navy #1124C, Gold #ffc20e) and secondary colors (White #ffffff, Gray #58595b), **When** a developer builds UI components, **Then** they reference theme variables instead of hardcoded color values
...
- **FR-002**: System MUST define brand color variables accessible to all frontend applications: Primary colors (Navy #1124C, Gold #ffc20e) and Secondary colors (White #ffffff, Gray #58595b)
- **FR-003**: System MUST apply navy background (#1124C) as the default background across all frontend applications (admin PWA, donor PWA, landing site)
- **FR-004**: System MUST provide logo export functionality for email templates with proper hosting or embedding support
- **FR-005**: System MUST generate and configure favicon files in multiple sizes (16x16, 32x32, 192x192, 512x512) for browser tabs, bookmarks, and mobile home screen icons
...
- **Theme Configuration**: Represents centralized color, typography, and spacing standards
  - Primary Colors: Navy (#1124C), Gold (#ffc20e)
...
- **Favicon Set**: Represents browser icon files in multiple sizes
  - Sizes: 16x16 (browser tab), 32x32 (taskbar), 192x192 (Android), 512x512 (iOS)
  - Formats: ICO (legacy), PNG (modern), SVG (progressive enhancement)
...
- **SC-002**: All UI components across all frontend applications display navy background (#1124C) consistently with no hardcoded color variations
...
- The Navy color (#1124C) is assumed to be correct, though the hex code appears truncated (may be #011124C or similar - requires clarification)
...
- **Color Code Typo**: The Navy color hex code (#1124C) appears to be missing a digit - should be verified before implementation
...
- The Navy hex code (#1124C) appears truncated and should be verified. Common navy shades are 6-digit codes like #011124, #001F3F, or #000080. This should be confirmed before implementation begins.
...
- The shared assets directory should be structured as: `frontend/shared/src/assets/logos/` and `frontend/shared/src/themes/`
