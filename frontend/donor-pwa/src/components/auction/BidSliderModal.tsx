...
export function BidSliderModal({
  ...
  const safeMinNextBid =
    parsedMinNextBid > 0 ? parsedMinNextBid : safeCurrentBid + safeBidIncrement
  ...
  const wheelOptions = useMemo(() => {
    ...
    for (
      let amount = safeMinNextBid;
      amount <= maxSliderAmount;
      amount += safeBidIncrement
    ) {
      values.push({
        value: amount,
      ...
      >
        ...
        <div className='space-y-6 py-4'>
          ...
          <div className='flex items-center gap-6'>
            {/* Vertical slider */}
            <div className='relative flex h-80 flex-col items-center'>
              {/* Amount display above slider */}
              <div
                className='mb-4 min-w-[120px] rounded-lg px-4 py-3 text-center'
                style={{
              ...
              >
                <WheelPickerWrapper className='h-44'>
                  <WheelPicker
                    ...
                    classNames={{
                      optionItem:
                        'text-sm text-center text-[var(--event-text-muted-on-background,#6B7280)]',
                      highlightWrapper:
