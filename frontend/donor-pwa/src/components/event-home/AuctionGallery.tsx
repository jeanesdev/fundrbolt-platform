...
export function AuctionGallery({
  ...
  const { mutate: addToWatchList } = useMutation({
    mutationFn: (itemId: string) =>
      watchListService.addToWatchList(eventId, itemId),
  ...
  const { mutate: removeFromWatchList } = useMutation({
    mutationFn: (itemId: string) =>
      watchListService.removeFromWatchList(eventId, itemId),
    onSuccess: (_data, itemId) => {
      queryClient.setQueryData(
        ['watchlist', eventId, watchlistScope],
        (
          ...
          const next = existing.filter(
            (entry) => entry.auction_item_id !== itemId
          )
          return {
            watch_list: next,
            total: next.length,
  ...
  useEffect(() => {
    ...
    const urlsToPreload = allLoadedItems
      ...
      .filter(
        (url) => !preloadedImageUrlsRef.current.has(url) && !warmCache.has(url)
      )
  ...
  if (effectiveUserId) {
    winningHistoryQueries.forEach((queryResult, index) => {
      ...
      const leadingBid = activeOrWinningBids.reduce(
        (currentLeader, candidate) => {
          ...
          return candidatePlacedAt >= currentPlacedAt
            ? candidate
            : currentLeader
        }
      )
  ...
  if (isError) {
    return (
      ...
      >
        <p className='text-destructive text-sm'>Failed to load auction items</p>
        <button
          onClick={() => refetch()}
              ...
              <Select value={categoryFilter} onValueChange={setCategoryFilter}>
                <SelectTrigger
                  ...
                  style={{
                    ...
                    borderColor:
                      'rgb(var(--event-primary, 59, 130, 246) / 0.4)',
                    color: 'var(--event-text-on-background, #111827)',
                  }}
                >
