...
const BID_STATUS_CLOSED_LABELS: Record<
  string,
...
function BidHistorySheet({
  bid,
  open,
  onOpenChange,
  isAuctionClosed,
  onViewItem,
}: BidHistorySheetProps) {
  if (!bid) return null
            ...
            <div className='min-w-0 flex-1'>
              ...
              {onViewItem && (
                <button
                  ...
                  onClick={() => {
                    onOpenChange(false)
                    onViewItem(bid.auction_item_id)
                  }}
                >
            ...
            {(() => {
              ...
              for (const entry of [...bid.bid_history].reverse()) {
                ...
                const effectiveStatus =
                  entry.outbid_by_bidder_number != null
                    ? 'outbid'
                    : entry.bid_status
                const existingStatus = existing
                  ? existing.outbid_by_bidder_number != null
                    ? 'outbid'
                    : existing.bid_status
                  : ''
                if (
              ...
              return dedupedHistory.map((entry, idx) => {
                const effectiveStatus =
                  entry.outbid_by_bidder_number != null
                    ? 'outbid'
                    : entry.bid_status
                const statusInfo = labels[effectiveStatus] ?? {
...
export function MyBidsDonationsSection({
  activity,
  isAuctionClosed = false,
  onViewItem,
}: MyBidsDonationsSectionProps) {
  const getStatusInfo = (status: string) => {
    const labels = isAuctionClosed
      ? BID_STATUS_CLOSED_LABELS
      : BID_STATUS_LABELS
  ...
  const totalBid = activity.bids
    .filter(
      (b) =>
        b.latest_bid_status === 'winning' || b.latest_bid_status === 'active'
    )
    .reduce((s, b) => s + Number(b.latest_bid_amount), 0)
  const totalDonation = activity.donations.reduce(
                      ...
                      <table className='w-full text-sm'>
                        <thead>
                          <tr
                            style={{
                              backgroundColor:
                                'rgb(var(--event-primary, 59, 130, 246) / 0.06)',
                            }}
                          >
                            <th
                          ...
                          {filteredBids.map((bid) => {
                            ...
                            return (
                              ...
                              >
                                ...
                                <td className='max-w-xs px-3 py-2'>
                                  <div className='flex items-center gap-2'>
                                    ...
                                    <span
                                      ...
                                      style={{
                                        color:
                                          'var(--event-text-on-background, #111827)',
                                      }}
                                    >
                                      {bid.item_title}
                                    </span>
                                ...
                                >
                                  {fmtCurrency(bid.latest_bid_amount)}
                                </td>
