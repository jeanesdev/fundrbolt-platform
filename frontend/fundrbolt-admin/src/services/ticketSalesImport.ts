...
export async function preflightTicketSalesImport(
  ...
  const response = await apiClient.post<PreflightResult>(
    `/api/v1/admin/events/${eventId}/ticket-sales/import/preflight`,
    formData,
    {
      headers: {
...
export async function commitTicketSalesImport(
  ...
  const response = await apiClient.post<ImportResult>(
    `/api/v1/admin/events/${eventId}/ticket-sales/import`,
    formData,
    {
      headers: {
        'Content-Type': 'multipart/form-data',
 ...
 */
export const EXAMPLE_CSV = `event_id,ticket_type,purchaser_name,purchaser_email,quantity,total_amount,purchase_date,external_sale_id,fee_amount,payment_status,notes
EVT-2026-001,VIP Table,Jordan Lee,jordan.lee@example.org,2,500.00,2026-02-01,EXT-100045,15.00,Paid,Sponsor package
EVT-2026-001,General Admission,Alex Smith,alex.smith@example.com,1,50.00,2026-02-02,EXT-100046,2.00,Paid,`
