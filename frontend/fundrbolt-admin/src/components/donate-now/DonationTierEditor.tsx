...
const DEFAULT_TIERS: TierDraft[] = [
  { amount_cents: 2500, impact_statement: '', display_order: 0 },
  { amount_cents: 5000, impact_statement: '', display_order: 1 },
  { amount_cents: 10000, impact_statement: '', display_order: 2 },
  { amount_cents: 25000, impact_statement: '', display_order: 3 },
]
...
export function DonationTierEditor({ npoId, tiers }: DonationTierEditorProps) {
    ...
    onSuccess: (data) => {
      queryClient.setQueryData(['donate-now-config', npoId], (old: unknown) => {
        if (old && typeof old === 'object')
          return { ...(old as object), tiers: data }
        return old
      })
      setDrafts(
        data.map((t, index) => ({
  ...
  const addTier = () => {
    ...
    setDrafts((prev) => [
      ...prev,
      { amount_cents: 1000, impact_statement: '', display_order: prev.length },
    ])
  }
    ...
    <div className='space-y-4'>
      {drafts.length === 0 && (
        <p className='text-muted-foreground text-sm'>
          No donation tiers yet. Add up to 10.
        </p>
      )}
      ...
      {drafts.map((tier, index) => (
        ...
        >
          <GripVertical className='text-muted-foreground mt-3 h-4 w-4 shrink-0' />
          <div className='flex flex-1 flex-col gap-2 sm:flex-row'>
            ...
            <div className='flex-1 space-y-1'>
              ...
              <Input
                ...
                onChange={(e) =>
                  updateTier(index, 'impact_statement', e.target.value)
                }
