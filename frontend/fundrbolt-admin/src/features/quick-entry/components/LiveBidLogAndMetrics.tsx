...
export function LiveBidLogAndMetrics({
  ...
  const winnerBid = useMemo(() => bids.find((b) => b.status === 'winning'), [bids])
  ...
  const filteredAndSortedBids = useMemo(() => {
    ...
    let rows = bids.filter((b) => {
      const timeText = new Date(b.accepted_at).toLocaleTimeString().toLowerCase()
      if (tableFilters.amount && !String(b.amount).includes(tableFilters.amount.replace(/,/g, ''))) return false
      if (tableFilters.bidder && !String(b.bidder_number).includes(tableFilters.bidder)) return false
      if (tableFilters.donor && !normalize(b.donor_name).includes(tableFilters.donor.toLowerCase())) return false
      if (tableFilters.table && !normalize(b.table_number).includes(tableFilters.table.toLowerCase())) return false
      if (tableFilters.time && !timeText.includes(tableFilters.time.toLowerCase())) return false
      if (tableFilters.status && !normalize(b.status).includes(tableFilters.status.toLowerCase())) return false
      return true
    })
    if (tableSort) {
      rows = [...rows].sort((a, b) => {
        ...
        else if (tableSort.field === 'bidder') cmp = a.bidder_number - b.bidder_number
        else if (tableSort.field === 'donor') cmp = (a.donor_name ?? '').localeCompare(b.donor_name ?? '')
        else if (tableSort.field === 'table') cmp = (a.table_number ?? '').localeCompare(b.table_number ?? '')
        else if (tableSort.field === 'time') cmp = new Date(a.accepted_at).getTime() - new Date(b.accepted_at).getTime()
        else if (tableSort.field === 'status') cmp = a.status.localeCompare(b.status)
        ...
        <div className="flex items-center justify-between rounded-md border border-yellow-400 bg-yellow-50 px-3 py-2 dark:border-yellow-600 dark:bg-yellow-950">
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm font-medium">
              Winner: Bidder #{winnerBid.bidder_number} — ${winnerBid.amount.toLocaleString('en-US')}
            </span>
          </div>
          <button
            type="button"
            ...
            onClick={() => {
              if (window.confirm('Remove winner? Bidding will reopen for this item.')) {
                onRemoveWinner()
              }
            }}
