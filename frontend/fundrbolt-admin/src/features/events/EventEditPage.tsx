...
export function EventEditPage() {
  ...
  const loadEvent = useCallback(() => {
    if (eventId) {
      ...
      // Check if eventId is a UUID or a slug (numeric IDs are legacy)
      const isId = /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i.test(eventId) || /^\d+$/.test(eventId)
      const loadFn = isId ? loadEventById : loadEventBySlug
