...
export function BuyNowEditor({
    ...
    onError: (error) => {
      toast.error(
        error instanceof Error
          ? error.message
          : 'Failed to update buy-now settings'
      )
    },
  })
  ...
  const hasChanges =
    enabled !== item.buy_now_enabled || quantity !== item.quantity_available
    ...
    <div className='space-y-4'>
      ...
      {!item.buy_now_price && (
        <Alert>
          ...
          <AlertDescription>
            No buy-now price set. Enable buy-now by editing the item and setting
            a buy-now price.
          </AlertDescription>
        </Alert>
      )}
      ...
      {showZeroQuantityWarning && (
        <Alert variant='destructive'>
          ...
          <AlertDescription>
            Quantity is zero. Buy-now will be disabled for donors even if
            enabled here.
          </AlertDescription>
        </Alert>
      )}
      ...
      {showLowQuantityWarning && (
        <Alert>
          <AlertCircle className='h-4 w-4' />
          <AlertDescription>
            Low quantity warning: Only {quantity} item
            {quantity !== 1 ? 's' : ''} remaining
          </AlertDescription>
        </Alert>
      )}
      ...
      <div className='bg-muted rounded-md p-3'>
        ...
        <div className='space-y-1 text-sm'>
          <div className='flex justify-between'>
            ...
            <span
              className={
                item.buy_now_enabled ? 'text-green-600' : 'text-red-600'
              }
            >
      ...
      <div className='flex justify-end gap-2'>
        ...
        <Button
          onClick={handleSave}
          disabled={
            !hasChanges || !isQuantityValid || updateBuyNowMutation.isPending
          }
        >
