...
export function TicketPackagesIndexPage({
  eventId: propEventId,
}: TicketPackagesIndexPageProps = {}) {
  // Get real event UUID from workspace context (URL param may be a slug)
  const { currentEvent } = useEventWorkspace()
  const routeParams = useParams({ strict: false }) as { eventId?: string }
  const eventId = propEventId || currentEvent.id || routeParams.eventId || ''
  ...
  return (
    <div className='space-y-6'>
      <div>
        <h1 className='text-3xl font-bold'>Ticket Packages</h1>
        <p className='text-muted-foreground'>
          Create and manage ticket packages for this event
        </p>
      </div>
    ...
    >
      <CardHeader>
        <div className='flex items-start gap-2'>
          {/* Drag Handle */}
          <button
            className='text-muted-foreground hover:text-foreground mt-1 cursor-grab transition-colors active:cursor-grabbing'
            {...attributes}
            {...listeners}
          ...
          <div className='flex-1'>
            <div className='flex items-start justify-between'>
              ...
              <div className='hidden gap-1 sm:flex'>
                ...
                {!pkg.is_enabled && <Badge variant='secondary'>Disabled</Badge>}
                {pkg.is_sold_out && (
      ...
      <CardContent>
        ...
        {pkg.sold_count > 0 && (
          <Collapsible
            open={isExpanded}
            onOpenChange={() => onToggleExpand(pkg.id)}
          >
            ...
            <CollapsibleContent className='mt-3'>
              <PurchasersList eventId={eventId} packageId={pkg.id} />
            </CollapsibleContent>
          </Collapsible>
        )}
      </CardContent>
